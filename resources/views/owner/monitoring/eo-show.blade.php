{{-- resources/views/owner/monitoring/eo-show.blade.php --}}
...
<div class="max-w-7xl mx-auto px-4 py-6" style="font-family:'Poppins',sans-serif;">
    ...
    {{-- ===================== TREN PENDAPATAN + ACTIVITY FEED (2 kolom) ===================== --}}
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
        ...
        {{-- ACTIVITY FEED KHUSUS EO INI (dikirim controller sebagai $activities, sebelumnya belum ditampilkan) --}}
        <div class="card rounded-xl p-4 flex flex-col" style="max-height: 260px;">
            <h2 class="text-xs font-bold mb-3 flex items-center gap-1.5" style="color:#111827;">
                <svg class="w-3.5 h-3.5" style="color:#f97316;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg>
                Aktivitas Terbaru EO Ini
            </h2>

            <div class="flex-1 overflow-y-auto pr-1 space-y-2.5">
                @forelse ($activities as $act)
                    @php
                        $typeStyle = match ($act->type) {
                            'event_done'      => ['#f0fdf4', '#16a34a', '✅'],
                            'withdrawal'       => ['#eff6ff', '#2563eb', '💸'],
                            'withdrawal_merch'  => ['#eff6ff', '#2563eb', '📦'],
                            'topup'            => ['#fffbeb', '#d97706', '➕'],
                            'refund_batch'     => ['#fef2f2', '#dc2626', '🔁'],
                            'user_refund'      => ['#fef2f2', '#dc2626', '↩️'],
                            default            => ['#f3f4f6', '#6b7280', '•'],
                        };
                    @endphp
                    <div class="flex items-start gap-2">
                        <span class="shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-[11px]"
                              style="background:{{ $typeStyle[0] }};">
                            {{ $typeStyle[2] }}
                        </span>
                        <div class="min-w-0">
                            <p class="text-[11px] leading-snug" style="color:#374151;">{{ $act->message }}</p>
                            <p class="text-[9px] mt-0.5" style="color:#9ca3af;">
                                {{ \Carbon\Carbon::parse($act->event_time)->diffForHumans() }}
                            </p>
                        </div>
                    </div>
                @empty
                    <p class="text-center text-xs py-6" style="color:#9ca3af;">Belum ada aktivitas tercatat.</p>
                @endforelse
            </div>
        </div>
...
<script>
(function () {
    const summaryUrlBase = "{{ url('/owner/monitoring/event') }}";
