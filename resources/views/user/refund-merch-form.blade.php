...
@section('title', 'Pengajuan Refund Merchandise')
...
<div class="px-6 lg:px-16 xl:px-24 2xl:px-32 py-8 bg-light">
    <div class="row justify-content-center">
        <div class="col-md-8 col-lg-6">
            ...
            {{-- TOMBOL KEMBALI --}}
            <a href="{{ route('user.merch') }}" class="btn btn-sm btn-white border rounded-pill px-3 mb-4 shadow-sm text-dark">
            ...
            <div class="card border-0 shadow-sm rounded-4 overflow-hidden bg-white">
                ...
                <div class="card-body p-4 p-md-5">
                    ...
                    <div class="p-3 rounded-4 bg-light mb-4 border border-light">
                        ...
                        <h5 class="fw-bold text-dark mb-1">{{ $transaction->event_title ?? 'Merchandise Event' }}</h5>
                        <div class="d-flex justify-content-between text-muted small mt-2 pt-2 border-top">
                            ...
                            <span>Total Bayar: <strong class="text-orange">Rp {{ number_format($transaction->total_amount, 0, ',', '.') }}</strong></span>
                        </div>
                    </div>
                    ...
                    <div class="alert alert-warning border-0 rounded-4 p-3 mb-4 small text-dark d-flex gap-2">
                        ...
                        <div>
                            ...
                            <ul class="mb-0 ps-3 mt-1">
                                <li>Dana akan dikembalikan senilai <span class="fw-bold">Rp {{ number_format($transaction->total_amount, 0, ',', '.') }}</span>.</li>
                    ...
                    {{-- ALUR INPUT FORM --}}
                    <form action="{{ route('user.merch-refund.store', $transaction->id) }}" method="POST">
                        @csrf
                        ...
                        <div class="mb-4">
                            ...
                            <textarea name="refund_reason" 
                                      ...
                                      rows="3" 
                                      placeholder="Tulis alasan pembatalan merchandise jika diperlukan...">{{ old('refund_reason') }}</textarea>
                            @error('refund_reason')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
